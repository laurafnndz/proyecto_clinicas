...
with
source as (
    select * from {{ source('bronze_clinicas', 'hospitalizaciones') }}
),
duenos as (
    select id_dueno, dni
    from {{ ref('stg__dueno') }}
    {# en hospitalizaciones no tengo id_dueno, tengo dni, por eso el join con stg__dueno
       para poder tener id_dueno y hacer el join con stg__mascota correctamente
...
{% if is_incremental() %}
where fecha_ingreso > (select max(fecha_ingreso) from {{ this }})
{% endif %}